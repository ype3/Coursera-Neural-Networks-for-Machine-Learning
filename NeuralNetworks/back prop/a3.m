...
function test_gradient(model, data, wd_coefficient)
  ...
  % We want to test the gradient not for every element of theta, because that's a lot of work. Instead, we test for only a few elements.
  % If there's an error, this is probably enough to find that error.
